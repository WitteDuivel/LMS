from lmsmodules import Book, BorrowersRecords, ReturnRecords
...
def login(username = None, user_passwd = None) -> bool:
	...
	while True:
		if username is None and user_passwd is None:
			...
			if librarian_id is not None and lib_passwd is not None:
				# todo query to check the credentials of the librarian against the database
				verification = True  # fixme if credentials verify against the database return True otherwise return false
				if verification:
					librarianMethods(librarian_id, lib_passwd)
			else:
...
dict_of_actions = {1: userProfileOverview, 2: userProfileEdit, 3: Book.dueDate, 4: Book.reservationStatus,
                   5: BorrowersRecords.calculateFine, 6: Book.giveBook, 7: Book.editBookDatabase,
                   8: BorrowersRecords.overview, 9: ReturnRecords.overview}
